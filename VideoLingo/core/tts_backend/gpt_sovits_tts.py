...
def gpt_sovits_tts(text, text_lang, save_path, ref_audio_path, prompt_lang, prompt_text):
    ...
    response = requests.post(f'{gpt_sovits_url}/tts', json=payload)
...
def gpt_sovits_tts_for_videolingo(text, save_as, number, task_df):
    import os
    if not os.environ.get('DOCKER_MODE') == '1':
...
def find_and_check_config_path(dubbing_character):
    ...
    if os.environ.get('DOCKER_MODE') == '1':
        gpt_sovits_dir = Path('/workspace/GPT-SoVITS')
    else:
        current_dir = Path(__file__).resolve().parent.parent.parent
        parent_dir = current_dir.parent
        gpt_sovits_dir = next((d for d in parent_dir.iterdir() if d.is_dir() and d.name.startswith('GPT-SoVITS')), None)
        if gpt_sovits_dir is None:
            raise FileNotFoundError("GPT-SoVITS directory not found in the parent directory.")
